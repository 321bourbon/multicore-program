...
__global__ void stereo_bm_cuda_sad4(const unsigned char* left, const unsigned char* right, unsigned char* disp,
...
int main(int argc, char* argv[]){
    //运行示例：./cuda_v4 left.pgm right.pgm disp_cuda_v4.pgm 128 17
    //disp_cuda_v4.pgm：输出视差图；128：最大视差；17：窗口大小
    if(argc<6){
        printf("Example:%s left.pgm right.pgm disp_cuda_v4.pgm 128 17\n",argv[0]);
        return 1;
    }
    ...
    stereo_bm_cuda_sad4<<<grid,block>>>(d_left, d_right, d_disp,
    ...
    printf("CUDA4 SAD4 StereoBM time: %.3f ms\n", time_ms);
